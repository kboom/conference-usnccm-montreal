...
\begin{itemize}
  \item Background
  \item Isogeometric L2 projections algorithm - theory
  \item Isogeometric L2 projections algorithm - algorithm
  \item Isogeometric L2 projections algorithm - implementation
  \item Conclusions
\end{itemize}
...
\begin{frame}{Algorithm - applications}
...
This \textbf{twofold scalability} greatly broadens the range of problems which can be solved using this technique. This is also relevant to time-dependent problems.
...
CUDA (GPU):
\begin{itemize}
  \item extremely fast but only for problems which fit in integrated memory
  \item low-level and fragile
  \item not portable
\end{itemize}
...
\begin{itemize}
  \item fast but only for problems which fit in RAM ($6144x6144=37$ million elements requires 12GB of RAM)
...
\begin{itemize}
  \item slower than both GPU and shared memory version but can solve much larger problems which do not fit into a physical memory of single machine
  \item scales both up and out
\end{itemize}
...
\begin{frame}{Implementation - IMDG}

\vskip 0.1in

   \begin{figure}
      \centering
      \includegraphics[width=0.8\textwidth]{img/hazelcast}
    \end{figure}
...
\begin{itemize}
  \item split tree into subtrees of a given height and store them on a single node (region-height)
  \item reuse same operation on multiple vertices (max-batch-size)
  \item run subsequent operations applied on same vertex in one invocation
  \item schedule tasks in batches (max-job-count)
\end{itemize}
...
\begin{frame}{Implementation - partition tree}
   ...
   \begin{figure}
      \centering
      \includegraphics[width=0.9\textwidth]{img/graph-regions}
    \end{figure}
...
\begin{frame}{Implementation - performance results}
...
\begin{itemize}
  \item Distributed memory implementation is much slower than shared memory one for relatively small problem sizes which fit into physical memory of a single machine
  \item Given there is a shared memory machine with sufficient physical memory, IMDG implementation is expected to outperform it starting from a certain problem
  \item This implementation can be made significantly faster by pushing some logic into worker nodes
\end{itemize}

\begin{figure}
      \centering
      \includegraphics[width=0.6\textwidth]{img/hazelcast-embedded}
    \end{figure}
